$(document).ready(function() {
  ...
  socket.on('postMyMessage', data => {
    ...
    $('section div').html(`
        ${$('section div').html()}
        <p class="messages messagesStyling userMessage"><small class="info"><span class="senderStyling">${$(
          '.userName'
        ).html()}</span>${data.stamp}</small class="info">${data.content}</p>
        `);
  });
  socket.on('postMessage', data => {
    ...
    $('section div').html(`
        ${$('section div').html()}
        <p class="messages messagesStyling"><small class="info"><span class="senderStyling">${
          data.sender
        }</span>${data.stamp.date}</small class="info">${data.content}</p>
        `);
  });
